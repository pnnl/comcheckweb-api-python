...
updated_project = project_building_area_operations.add_building_area_to_project(
    project, new_building_area  # type: ignore[arg-type] - project is ComBuilding | None, expects ComBuilding
)
client.update_project(str(updated_project.id), updated_project)

# Example 2: Update a building area
building_area_key = str(project.lighting.wholeBldgUse[0].key)  # type: ignore[union-attr] - project is ComBuilding | None, accessing attr on None
updates = {
...
updated_project = project_building_area_operations.update_building_area_in_project(
    project, building_area_key, updates  # type: ignore[arg-type] - project is ComBuilding | None, expects ComBuilding
)
client.update_project(str(updated_project.id), updated_project)

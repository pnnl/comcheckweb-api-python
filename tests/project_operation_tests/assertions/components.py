...
from comcheck_api.utilities.project_utilities import (
    find_component_in_component_list,
...
def assert_component_added(
    ...
    assert isinstance(components, list) and components, (
        f"Expected at least one {component_name} at path " f"'{path}' after add"
    )

    added_component = find_component_in_component_list(
        components=components, component_id=component_id
    )
...
def assert_component_updated(
    ...
    components = project.get_by_path(path, [])
    assert isinstance(components, list) and components, (
        f"Expected at least one {component_name} at path " f"'{path}' after update"
    )

    updated_component = find_component_in_component_list(
        components=components, component_id=component_id
    )
...
def assert_component_removed(
    ...
    components: list[CustomBaseModel] = project.get_by_path(path, []) or []
...
def assert_component_has_id(
    ...
    assert id is not None, f"Component {component} has no identifier"
    return id
